...
<body class="tankBody" onload="setListeners()">
...
<header>
    <h3>
        <img id="titleLogo" src="./assets/logo/logo_white_large.png" alt="THE AQUARIUM"> <c:out value="${tank.name}"/>
    </h3>
    <p>
        ...
        <c:choose>
            <c:when test="${sensor.classDescriptor.name() eq 'OXYGEN'}">
                <li <c:out value="id=${sensor.liID}"/>>
                    <h4><c:out value="${sensor.classDescriptor.name()}"/></h4>
                    <div class="deviceContainer">
                        ...
                        <button class="plotButton" <c:out value="id=${sensor.plotButtonID}"/> type="button">Plot</button>
                    </div>
                    <c:forEach var="tempAct" items="${acts}">
                        <div class="deviceContainer" <c:out value="id=${tempAct.identifier}"/> >
                                ...
                                <label class="toggle"> Toggle:
                                    <c:choose>
                                        <c:when test="${tempAct.currentValue eq 0}">
                                            <%-- OFF --%>
                                            <input
                                                <c:out value="id=${tempAct.toggleID}"/> type="checkbox" class="toggle">
                                        </c:when>
                                        <c:otherwise> <%-- ON --%>
                                            <input
                                                <c:out value="id=${tempAct.toggleID}"/> class="toggle" type="checkbox"
                                                                                        checked>
                                        </c:otherwise>
                                    </c:choose>
                                    <span></span>
                    ...
                    <div class="plot" <c:out value="id=${sensor.identifier}"/> hidden></div>
                </li>
            </c:when>
                                ...
                                <c:if test="${tempAct != null}">
                                    ...
                                    <label class="toggle"> Toggle:
                                        <c:choose>
                                            <c:when test="${tempAct.currentValue eq 0}"> <%-- OFF --%>
                                                <input
                                                    <c:out value="id=${tempAct.toggleID}"/> type="checkbox"
                                                                                            class="toggle">
                                            </c:when>
                                            <c:otherwise> <%-- ON --%>
                                                <input
                                                    <c:out value="id=${tempAct.toggleID}"/> class="toggle"
